...
export function AuthPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (isDemoFlow && !activeIsDemo) {
        setMessage('You are signed in to a private pilot account. Sign out to open the sample walkthrough.');
        return;
      }
      if (!isDemoFlow && activeIsDemo) {
        setMessage('You are signed in with the sample walkthrough account. Sign out before creating or testing a real pilot.');
        return;
      }
      if (data.session) {
  ...
  async function signOutCurrentSession() {
    ...
    if (isDemoFlow) {
      setEmail(demoEmail);
      setPassword(demoPassword);
      setMessage('Signed out of the private pilot account. Continue with the sample walkthrough account.');
    } else {
      setEmail('');
      setPassword('');
      setMessage('Signed out of the sample walkthrough. Continue with your pilot account.');
    }
  }
  ...
  async function submitAuth(event?: FormEvent<HTMLFormElement>) {
    ...
    if (sessionFlowMismatch) {
      setMessage(
        isDemoFlow
          ? 'Sign out of the private pilot account before opening the sample walkthrough.'
          : 'Sign out of the sample walkthrough before using a real pilot.'
      );
      return;
    }
        ...
        <div className="auth-card">
          ...
          {isDemoFlow ? (
            <div className="notice auth-context">
              <strong>Sample walkthrough</strong>
              <span>
          ...
          <form onSubmit={submitAuth}>
            ...
            <button className="btn primary" style={{ width: '100%' }} type="submit">
              {isDemoFlow ? 'Open sample walkthrough' : mode === 'sign-in' ? 'Continue to pilot' : 'Create pilot account'}
            </button>
          </form>
          {sessionFlowMismatch ? (
            <button className="btn" style={{ width: '100%', marginTop: 10 }} type="button" onClick={signOutCurrentSession}>
              Sign out of {signedInAsDemo ? 'sample walkthrough' : 'private pilot account'}
            </button>
          ) : null}
          <p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>
            {isDemoFlow
              ? 'The sample walkthrough account is prefilled and intentionally separate from real pilot accounts.'
              : 'Use this path for your own app data and setup. To inspect sample data, open the separate sample walkthrough.'}
          </p>
          <div className="auth-links">
            <ButtonLink to="/">Back to landing</ButtonLink>
            {isDemoFlow ? <ButtonLink to="/onboarding">Start real pilot</ButtonLink> : <ButtonLink to="/demo">View sample walkthrough</ButtonLink>}
          </div>
        </div>
      </Card>
    </div>
