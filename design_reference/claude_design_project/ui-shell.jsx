  ...
  return (
    <div style={{ width: '100%', height: '100%', background: U.bg, color: U.ink, fontFamily: U.sans, fontSize: 13, display: 'flex', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0 }}>
        ...
        <div className="u-fade-in" key={route} style={{ flex: 1, overflow: 'auto', background: U.bg }}>
...
function RunRow({ label, value }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between' }}>
      <span style={{ color: U.ink3 }}>{label}</span>
      <span style={{ fontFamily: U.mono, fontSize: 11.5, color: U.ink2 }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ height: 56, borderBottom: '1px solid '+U.line, background: U.surface, display:'flex', alignItems:'center', padding:'0 22px', gap: 16, flexShrink: 0 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap: 8, fontSize: 13 }}>
        ...
        <span style={{ color: U.ink3 }}>acme-copilot</span>
        <span style={{ color: U.faint }}>/</span>
      ...
      <button onClick={onCmd}
        style={{ display:'flex', alignItems:'center', gap: 10, padding:'5px 8px 5px 10px', height: 30, background: U.surface2, border: '1px solid '+U.line, borderRadius: 8, cursor:'pointer', fontFamily: U.sans, fontSize: 12.5, color: U.ink3, minWidth: 200 }}>
        ...
        <span>Search guards, cells…</span>
        <div style={{ flex: 1 }}/>
        <UKbd>⌘K</UKbd>
      </button>

      {/* λ slider */}
      <div style={{ display:'flex', alignItems:'center', gap: 10, padding:'5px 10px 5px 12px', height: 30, border: '1px solid '+U.line, borderRadius: 8, background: U.surface2 }}>
        ...
        <span style={{ fontFamily: U.mono, color: U.ink, fontSize: 12, width: 18, textAlign:'right', fontVariantNumeric:'tabular-nums' }}>{lam}</span>
        <span style={{ color: U.ink3, fontSize: 11.5, fontStyle:'italic', minWidth: 70 }}>{lamPreset?.name || 'custom'}</span>
      </div>
        ...
        {personas.map(p => (
          <button key={p.id} onClick={() => setPersona(p.id)}
            style={{ background: persona===p.id ? U.surface : 'transparent', color: persona===p.id ? U.ink : U.ink3,
              ...
              border: 'none', padding: '0 11px', fontFamily: U.sans, fontSize: 12, fontWeight: 500, cursor:'pointer', borderRadius: 5, transition: 'all .1s' }}>
            {p.label}
          </button>
        ))}
      </div>

      <button title="Notifications" style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid '+U.line, background: U.surface, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
